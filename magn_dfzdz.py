...
def dfz_dz (mx, my, mz, xnodes, ynodes, znodes, xstepsize, ystepsize, zstepsize, Z):
    ...
    for k in range(0, znodes):
        ...
        for i in range(0, xnodes):
            ...
            for j in range(0, ynodes):
                ...
                F[i,j]=F[i,j]+sum(sum(
                -15*(mx[k,:,:]*(X0-X)+my[k,:,:]*(Y0-Y)+mz[k,:,:]*(Z0+Z))*(Z0+Z)**2
                /((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(7/2)
                +3*(mx[k,:,:]*(X0-X)+my[k,:,:]*(Y0-Y))/((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(5/2)
                +9*mz[k,:,:]*(Z0+Z)/((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(5/2)))
    return F

def lost_dfz_dz (mx, my, mz, nx, ny, nz, xstepsize, ystepsize, zstepsize, Z, factor):
    xnodes=nx//factor
    ynodes=ny//factor
    znodes=nz
    X, Y = numpy.meshgrid(range(-xnodes//2, xnodes//2), range(-ynodes//2, ynodes//2))
    ...
    for k in range(0, znodes):
        ...
        for i in range(0, xnodes):
            ...
            for j in range(0, ynodes):
                ...
                F[i,j]=F[i,j]+sum(sum(
                -15*(mx2*(X0-X)+my2*(Y0-Y)+mz2*(Z0+Z))*(Z0+Z)**2
                /((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(7/2)
                +3*(mx2*(X0-X)+my2*(Y0-Y))/((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(5/2)
                +9*mz2*(Z0+Z)/((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(5/2)))
    return [F*factor*factor, xnodes, ynodes]

def f_dfz_dz (mx, my, mz, xnodes, ynodes, znodes, xstepsize, ystepsize, zstepsize, Z):
    X, Y = numpy.meshgrid(range(-xnodes//2, xnodes//2), range(-ynodes//2, ynodes//2))
    X=X*xstepsize
    Y=Y*ystepsize
...
#mx* -15*x*z**2/r2*(7/2)+3*x/r2**(5/2)
#my* -15*z**2*y/r2**(7/2)+3*y/r2**(5/2)
#mz* -15*z**3/r2**(7/2)+9*z/r2**(5/2)

        fx=numpy.fft.fft2(-15*X*(Z+Z0)**2/(X**2+Y**2+(Z+Z0)**2)**(7/2)+3*X/(X**2+Y**2+(Z+Z0)**2)**(5/2))
        fy=numpy.fft.fft2(-15*Y*(Z+Z0)**2/(X**2+Y**2+(Z+Z0)**2)**(7/2)+3*Y/(X**2+Y**2+(Z+Z0)**2)**(5/2))
        fz=numpy.fft.fft2(-15*(Z+Z0)**3/(X**2+Y**2+(Z+Z0)**2)**(7/2)+9*(Z+Z0)/(X**2+Y**2+(Z+Z0)**2)**(5/2))
