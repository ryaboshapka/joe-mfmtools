...
def d2fx_dz2 (mx, my, mz, xnodes, ynodes, znodes, xstepsize, ystepsize, zstepsize, Z):
    ...
    for k in range(0, znodes):
        ...
        for i in range(0, xnodes):
            ...
            for j in range(0, ynodes):
                ...
                F[i,j]=F[i,j]+sum(sum(
                105*(mx[k,:,:]*(X0-X)+my[k,:,:]*(Y0-Y)+mz[k,:,:]*(Z0+Z))*(X0-X)*(Z0+Z)**2
                /((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(9/2)
                -15*mx[k,:,:]*(Z0+Z)**2/((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(7/2)
                -15*(mx[k,:,:]*(X0-X)+my[k,:,:]*(Y0-Y)+mz[k,:,:]*(Z0+Z))*(X0-X)/((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(7/2)
                -30*mz[k,:,:]*(Z+Z0)*(X0-X)/((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(7/2)
                +3*mx[k,:,:]/((X-X0)**2+(Y-Y0)**2+(Z+Z0)**2)**(5/2)))
    return F
...
def f_d2fx_dz2 (mx, my, mz, xnodes, ynodes, znodes, xstepsize, ystepsize, zstepsize, Z):
    X, Y = numpy.meshgrid(range(-xnodes//2, xnodes//2), range(-ynodes//2, ynodes//2))
    X=X*xstepsize
    Y=Y*ystepsize
...
#mx* 105*x**2*z**2/r2**(9/2)-15*(x**2+z**2)/r2**(7/2)+3/r2**(5/2)
#my* 105*x*y*z**2/r2**(9/2)-15*x*y/r2**(7/2)
#mz* 105*x*z**3/r2**(9/2)-45*x*z/r2**(7/2)

        fx=numpy.fft.fft2(105*X**2*(Z+Z0)**2/(X**2+Y**2+(Z+Z0)**2)**(9/2)-15*(X**2+(Z+Z0)**2)/(X**2+Y**2+(Z+Z0)**2)**(7/2)+3/(X**2+Y**2+(Z+Z0)**2)**(5/2))
        fy=numpy.fft.fft2(105*X*Y*(Z+Z0)**2/(X**2+Y**2+(Z+Z0)**2)**(9/2)-15*X*Y/(X**2+Y**2+(Z+Z0)**2)**(7/2))
        fz=numpy.fft.fft2(105*X*(Z+Z0)**3/(X**2+Y**2+(Z+Z0)**2)**(9/2)-45*X*(Z+Z0)/(X**2+Y**2+(Z+Z0)**2)**(7/2))
